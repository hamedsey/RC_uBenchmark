...
struct resources {
    ...
    volatile char *buf4;                 // memory buffer pointer, used for
    volatile char *buf5;                 // memory buffer pointer, used for
    volatile char *buf6;                 // memory buffer pointer, used for
    volatile char *buf7;                 // memory buffer pointer, used for
    volatile char *buf8;                 // memory buffer pointer, used for
    volatile char *buf9;                 // memory buffer pointer, used for
    volatile char *buf10;                 // memory buffer pointer, used for
    volatile char *buf11;                 // memory buffer pointer, used for
    volatile char *buf12;                 // memory buffer pointer, used for
    */
...
static int resources_create(struct resources *res, uint32_t numberOfQueues, uint64_t numberOfThreads) {
    ...
    uint64_t offset = 16777216; //32*131072;//4096;//65536;
    /*
    ...
    */
    ...
    uint64_t numAllocatedBits;
    numAllocatedBits = numberOfQueues;
    //if(numberOfQueues < 8 == 0) numAllocatedBits = 8;
    //else if(numberOfQueues % 8 == 0) numAllocatedBits = numberOfQueues;
    //else numAllocatedBits = ((numberOfQueues/8) + 1)*8;

    //printf("number of bits allocated = %llu \n", numAllocatedBits);
    uint64_t totalBytes;
    ...
    res->buf = (volatile char *)memalign(offset, totalBytes * sizeof(volatile char));
    assert(res->buf != NULL);
    //res->buf2 = (volatile char *)memalign(4096, totalBytes * sizeof(volatile char));
    //assert(res->buf != NULL);
    ...
    //for(uint64_t s = 0; s < totalBytes; s++) printf("buf address %p \n",(res->buf)+s);
        ...
        /*
    while(reinterpret_cast<uintptr_t>(res->buf) % 0x10000 != 0) {
        //free((void*)(res->buf));
        assert(posix_memalign((void**)(&res->buf), offset, totalBytes) == 0);
        printf("again buf address %p \n",(res->buf));

    }
    */
    /*
    assert(posix_memalign((void**)(&res->buf2), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf2));

    assert(posix_memalign((void**)(&res->buf3), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf3));

    assert(posix_memalign((void**)(&res->buf4), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf4));

    assert(posix_memalign((void**)(&res->buf5), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf5));

    assert(posix_memalign((void**)(&res->buf6), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf6));

    assert(posix_memalign((void**)(&res->buf7), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf7));

    assert(posix_memalign((void**)(&res->buf8), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf8));

    assert(posix_memalign((void**)(&res->buf9), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf9));

    assert(posix_memalign((void**)(&res->buf10), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf10));

    assert(posix_memalign((void**)(&res->buf11), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf11));

    assert(posix_memalign((void**)(&res->buf12), offset, totalBytes) == 0);
        printf("buf address %p \n",(res->buf12));
    */
    //for(int i = 0 ; i < numberOfThreads; i++) printf("buf address [%llu] = %p \n", i, (res->buf[i]));
    memset((void*)(res->buf), 0x00,totalBytes);
    int t;
    //for(t = 0; t < totalBytes; t++) printf("%p , %llu \n",((res->buf)+t), (res->buf)[t]);
